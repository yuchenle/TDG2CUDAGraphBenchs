...
{
  printf("Compute_yi: Iteration number = %d\n", num_atoms * idxz_max);
...
#pragma omp parallel for collapse(2) default(none)
  ...
  int num_blocks = 4;
  int block_size = (idxz_max * num_atoms) / num_blocks;
...
#pragma omp parallel for collapse(2) default(none) shared(idxz,                \
  ...
  #endif
  for (int k = 0; k < idxz_max * num_atoms; k += block_size) {
    #pragma omp target nowait
    #pragma omp teams distribute parallel for
    for (int kk = k; kk < k+block_size; ++kk) {
      int jjz = kk / num_atoms;
      int natom = kk % num_atoms;
  ...
  spend = (end_time.tv_sec - start_time.tv_sec) * 1e6 + (end_time.tv_usec - start_time.tv_usec);
  printf("Compute_yi: takes %f us\n", spend);
...
#pragma omp for collapse(2)
  ...
  //   for (int natom = 0; natom < num_atoms; natom++) {
  int num_blocks = 4;
  printf("compute_duidrj: num_atomcs * num_nbor = %d\n", num_atoms * num_nbor);
  int block_size = (num_atoms * num_nbor) / num_blocks;
  gettimeofday(&start_time, NULL);
  #pragma omp parallel
  #pragma omp single
  {
  ...
  #pragma omp taskgraph target_graph
  #endif
  for (int k = 0; k < num_nbor * num_atoms; k += block_size) {
    #pragma omp target nowait
    #pragma omp teams distribute parallel for
    for (int kk = k; kk < k + block_size; kk++) {
      int nbor = kk / num_atoms;
      int natom = kk % num_atoms;
  ...
  gettimeofday(&end_time, NULL);
  spend = (end_time.tv_sec - start_time.tv_sec) * 1e6 + (end_time.tv_usec - start_time.tv_usec);
  printf("compute_duidrj: takes %f us\n", spend);
}
