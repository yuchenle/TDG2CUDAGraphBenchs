...
int64_t Nloop=1000;
  ...
  std::cout<<GridLogMessage << "----------------------------------------------------------"<<std::endl;
  // for(int lat=LMIN;lat<=LMAX;lat+=LADD){
  // changed the lower bound to LMAX, to see how the cuda graph performance is, without needing to implement
  // graph re-recording
  for(int lat=LMAX;lat<=LMAX;lat+=LADD){
     ...
     #pragma omp target enter data map(alloc:zv._odata[ :zv.size()]) \
      ...
      printf("Nwarm = %ld, vol = %ld, Nloop = %ld\n", Nwarm, vol, Nloop);
      for(int64_t i=0;i<Nwarm;i++){
...
#endif
      for(int64_t i=0;i<Nloop;i++){
        double inner_start=usecond();
#ifdef TASKGRAPH
        #pragma omp taskgraph target_graph
        {
#endif
          int numBlocks = 4; // assuming vol % numStreams = 0
          int blockSize = vol/numBlocks;
          int idx=0;
          for (int64_t s = 0; s < vol; idx++, s += blockSize) {
            #pragma omp target nowait // depend(inout:dep[idx % 16])
            #pragma omp teams distribute parallel for //thread_limit(gpu_threads)
            for (int64_t s2 = s; s2 < s + blockSize; s2++) {
              zv[s2]=xv[s2]*yv[s2];
            }
          }
#ifdef TASKGRAPH
        }
        double inner_stop=usecond();
        double time = (inner_stop-inner_start);
        // printf("iteration %d took %f us\n", i, time);
#endif
      }
      #ifndef TASKGRAPH
      #pragma omp taskwait
	...
	z=x*y;
      ...
      std::cout<<GridLogMessage<<"average time per iter " << std::setprecision(3) << time/1e6 << " ms" << std::endl;
