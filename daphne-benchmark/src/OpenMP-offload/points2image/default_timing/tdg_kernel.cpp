	...
	const ImageSize& imageSize)
{
	...
	#pragma omp target update from(distanceArr[:sizeMat],imagePointArr[:sizeMat])
    // #pragma omp taskwait
	// image formation
	for (uint32_t x = 0; x < cloudWidth; ++x) {
		for (uint32_t y = 0; y < cloudHeight; ++y) {
