...
// Defining Pointers to b used in hetero. graph here
// so that mem copy and host pointers they operate
// over constant addresses
float cloud[64000*32];
// graph host node always operate over the same pointer, therefore,
// we need to declare msg here, otherwise its address varies across iterations
PointsImage msg;
double total_time = 0;
...
int points2image::read_next_testcases(int count)
{
	...
	for (i = 0; (i < count) && (read_testcases < testcases); i++,read_testcases++)
	{
		try {
			...
			parseCameraMat(input_file, &cameraMat_g + i);
			parseDistCoeff(input_file, &distCoeff_g + i);
	...
	const ImageSize& imageSize, int iter)
{
	...
	// distanceArr = new double[sizeMat];
	// distanceArr = (double *)malloc(sizeof(double)*sizeMat);
	// imagePointArr = new Point2d[sizeMat];
	// imagePointArr = (Point2d *)malloc(sizeof(Point2d)*sizeMat);
	cloudHeight = pointcloud2.height;
	cloudWidth = pointcloud2.width;
	cloudStepSize = pointcloud2.point_step;

    START_TIMER;
	// point transformation
	#pragma omp target enter data map(to:distanceArr[:sizeMat],imagePointArr[:sizeMat], cloud[:sizeMaxCp],distCoeff_g,\
	                                     cameraMat_g,invT,invR,cloudHeight,cloudWidth,cloudStepSize) depend(out:distanceArr)

	// Why target enter data map does not update cloud, and we need an explicit target update...
	// #pragma omp target update to(cloud[:sizeMaxCp])
	// {}
	#ifdef BLOCKING
	int num_blocks = 32;
	int block_size = sizeMat / num_blocks;
	int dep_blocks[num_blocks];
	#ifdef TASKGRAPH
	// upper bound is 64000 max size for taskgraph to not miss any data
	#pragma omp taskgraph target_graph
	{
		#pragma omp target nowait depend(out:distanceArr) map(always,to:distanceArr[:sizeMat],imagePointArr[:sizeMat], cloud[:sizeMaxCp],distCoeff_g,\
	                                     cameraMat_g,invT,invR,cloudHeight,cloudWidth,cloudStepSize)
										 {}
	for (uint32_t k = 0, i = 0; k < sizeMat; k += block_size, ++i) {
	#else
	for (uint32_t k = 0; k < cloudWidth * cloudHeight; k += block_size) {
	#endif

		#pragma omp target nowait depend(out:dep_blocks[i]) depend(in:distanceArr)// \
		                   has_device_addr(distanceArr,imagePointArr, cloud,distCoeff, \
	...
	for (uint32_t x = 0; x < cloudWidth; ++x) {
		for (uint32_t y = 0; y < cloudHeight; ++y) {
			...
			double tmpdist = 1 + distCoeff_g.data[0] * r2
					+ distCoeff_g.data[1] * r2 * r2
					+ distCoeff_g.data[4] * r2 * r2 * r2;
			Point2d imagepoint;
			imagepoint.x = tmpx * tmpdist
				+ 2 * distCoeff_g.data[2] * tmpx * tmpy
				+ distCoeff_g.data[3] * (r2 + 2 * tmpx * tmpx);
			imagepoint.y = tmpy * tmpdist
				+ distCoeff_g.data[2] * (r2 + 2 * tmpy * tmpy)
				+ 2 * distCoeff_g.data[3] * tmpx * tmpy;
			imagepoint.x = cameraMat_g.data[0][0] * imagepoint.x + cameraMat_g.data[0][2];
			imagepoint.y = cameraMat_g.data[1][1] * imagepoint.y + cameraMat_g.data[1][2];
	...
	#pragma omp target nowait depend(in:dep_blocks[0:num_blocks])\
	                          depend(out:distanceArr) map(always, from:distanceArr[:sizeMat],imagePointArr[:sizeMat]) 
	{}

	// image formation
	#pragma omp task depend(in:distanceArr)
	for (uint32_t x = 0; x < cloudWidth; ++x) {
		for (uint32_t y = 0; y < cloudHeight; ++y) {
			int iPoint =x + y * cloudWidth;
			// restore values
