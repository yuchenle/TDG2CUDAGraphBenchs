...
// ---------------------------------------------
  int num_blocks = 25; //assuming nparticles_to_process % 100 = 0
  // printf("nparticles_to_process %d\n", nparticles_to_process); // 1E6
  int elems_per_block = nparticles_to_process/num_blocks;
  #pragma omp target enter data map(to: nfacets, ncollisions, nparticles)
#ifdef TASKGRAPH
  
  #pragma omp taskgraph target_graph
  {
#endif
  for (int i = 0; i < nparticles_to_process; i+= elems_per_block) {
    #pragma omp target nowait //map(tofrom: nfacets, ncollisions, nparticles)
    #pragma omp teams distribute parallel for //simd reduction(+: nfacets, ncollisions, nparticles)
    for (int pp = i; pp < i + elems_per_block; pp++) {
  ...
  } // end of target nowait
  // } // end of nparticles_to_process loop
#ifdef TASKGRAPH
  } //end of taskgraph
  #pragma omp target exit data map(from: nfacets, ncollisions, nparticles)
#else
  #pragma omp taskwait
#endif
...
void microscopic_cs_for_energy(const double* keys,
  ...
  while (p_energy < keys[ind] || p_energy >= keys[ind + 1]) {
    ...
    width = max_local(1, width / 2); // To handle odd cases, allows one extra walk
  }
