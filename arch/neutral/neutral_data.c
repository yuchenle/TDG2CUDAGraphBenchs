...
void initialise_neutral_data(NeutralData* neutral_data, Mesh* mesh) {
  ...
  const double local_particle_left_off = max_local(0.0, source_xpos - *rank_xpos_0);
  const double local_particle_bottom_off = max_local(0.0, source_ypos - *rank_ypos_0);
  const double local_particle_right_off =
      max_local(0.0, *rank_xpos_1 - (source_xpos + source_width));
  const double local_particle_top_off =
      max_local(0.0, *rank_ypos_1 - (source_ypos + source_height));
  const double local_particle_width =
      max_local(0.0, (*rank_xpos_1 - *rank_xpos_0) -
                   (local_particle_right_off + local_particle_left_off));
  const double local_particle_height =
      max_local(0.0, (*rank_ypos_1 - *rank_ypos_0) -
                   (local_particle_top_off + local_particle_bottom_off));
#if 0
