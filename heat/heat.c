        ...
        for (iter = 0; iter < param.maxiter; ++iter) {
          ...
          for (int ii = 0; ii < nbx; ii++) {
            for (int jj = 0; jj < nby; jj++) {
              ...
              int inf_j = 1 + jj * by;
              int sup_j = ((inf_j + by) < sizey - 1) ? inf_j + by : sizey - 1;
              // double (*XXX)[sizey] = (double (*)[sizey])param.u;
              // printf("u[%d][%d] = %f \n", inf_i, inf_j, XXX[inf_i][inf_j]);
              // printf("u[%d][%d] = %f \n", inf_i, inf_j, param.u[inf_i * sizex + inf_j]);
              
              // printf(" \t ?? = %f \n", param.u[inf_i * sizex + inf_j]);
#pragma omp task depend(in                                                     \
                        : u[inf_i - bx][inf_j], u[sup_i][inf_j],               \
                          u[inf_i][inf_j - by], u[inf_i][sup_j])               \
    depend(inout                                                               \
                ...
                for (int i = inf_i; i < sup_i; ++i) {
                  for (int j = inf_j; j < sup_j; ++j) {
                    ...
                    // printf("i = %d, j = %d, unew = %f, param.u[%d] = %f\n",
                          ...
                          //  u2[i][j - 1], u2[i][j + 1], u2[i - 1][j], u2[i + 1][j],
                          //  param.u[i * sizey + j - 1], param.u[i * sizey + j + 1],
                          //  param.u[(i - 1) * sizey + j], param.u[(i + 1) * sizey + j]);
                  }
                }
              }
            }
